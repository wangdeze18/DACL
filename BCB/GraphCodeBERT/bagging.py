...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train'):
        ...
        url_to_code={}
        with open('/'.join(index_filename.split('/')[:-1])+'/large_data.jsonl') as f:
            for line in f:
                line=line.strip()
                js=json.loads(line)
...
def calscores(args, model, tokenizer,test_data_file):
    ...
    fname = "scores"+ str(test_data_file[-5]) + ".npy"
    np.save(fname,logits[:,1])
    return logits,y_trues,eval_dataset
...
def test(args, model, tokenizer, best_threshold=0):
    logits0,_,eval_dataset = calscores(args, model, tokenizer,"dataset/test.txt")
    logits1,_,_ = calscores(args, model, tokenizer,"dataset/test0.txt")
    logits2,_,_ = calscores(args, model, tokenizer,"dataset/test1.txt")
    logits3,_,_ = calscores(args, model, tokenizer,"dataset/test2.txt")
    logits4,_,_ = calscores(args, model, tokenizer,"dataset/test3.txt")
    y_preds = logits0[:,1] + logits1[:,1] + logits2[:,1] + logits3[:,1] + logits4[:,1]>best_threshold * 5
    with open(os.path.join(args.output_dir,"predictions_all.txt"),'w') as f:
...
def main():
    ...
    results = {}
    if args.do_eval:
        checkpoint_prefix = 'checkpoint-best-f1/model_large.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
