...
def train(args, train_dataset, model, tokenizer):
    ...
    c0 = 901028 / len(train_dataset)
    train_data = train_dataset[0:int(c0 * len(train_dataset))]
    
    
    train_sampler = RandomSampler(train_data)
    train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size,num_workers=4)
    
    s_for_count = RandomSampler(train_dataset)
    t_for_count = DataLoader(train_dataset, sampler=s_for_count, batch_size=args.train_batch_size,num_workers=4)
    args.max_steps=args.epochs*len( t_for_count)/2
    ...
    for idx in range(args.epochs): 
        ...
        if args.pacing_function != "":
            ...
            if percent != now_percent:
                ...
                train_data = train_dataset[0:now_percent]
...
def test(args, model, tokenizer, best_threshold=0):
    ...
    #output result
    logits=np.concatenate(logits,0)
    fname = "scores"+ str(args.test_data_file[-5]) + ".npy"
    np.save(fname,logits[:,1])
    y_preds=logits[:,1]>best_threshold
    with open(os.path.join(args.output_dir,"predictions"+ str(args.test_data_file[-5]) +".txt"),'w') as f:
...
def main():
    ...
    if args.do_train:
        ...
        logger.info("***** pre Running training *****")
        train(args, order_dataset, model, tokenizer)

    # Evaluation
    results = {}
    if args.do_eval:
        checkpoint_prefix = 'checkpoint-best-f1/model_curri.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
