...
def train(args, train_dataset, model, tokenizer, pool):
    """ Train the model """
    c0 = 901028*0.1/len(train_dataset)
    print("c0 = ",c0)
    ...
    s_for_count = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(train_dataset)
    t_for_count = DataLoader(train_dataset, sampler=s_for_count, batch_size=args.train_batch_size)
    args.max_steps = args.epoch * len(t_for_count)/2
    args.save_steps = len(train_dataloader)
    args.warmup_steps = len(t_for_count)
...
def main():
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, order_dataset, model, tokenizer, pool)
