...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512,pool=None):
        ...
        if 'test' not in postfix:#if 'valid' in postfix:
            data=random.sample(data,int(len(data)*0.1))

        self.examples=pool.map(get_example,tqdm(data,total=len(data)))#[get_example(i) for i in tqdm(data,total=len(data))]#pool.map(get_example,tqdm(data,total=len(data)))
        '''
        if 'train' in postfix:
...
def test(args, model, tokenizer, prefix="",pool=None,best_threshold=0):
    ...
    y_preds=logits[:,1]>best_threshold
    with open(os.path.join(args.output_dir,"predictions"+ args.test_data_file[-5]+".txt"),'w') as f:
...
def main():
    ...
    if args.local_rank == -1 or args.no_cuda:
        ...
        #args.n_gpu = 4
        #os.environ['CUDA_VISIBLE_DEVICES'] = "3,4,5,6"#"8,9,10,11,12,13,14,15"
